...
var jush = {
    create_links: true,
    // string for extra <a> parameters, e.g. ' target="_blank"'
    timeout: 1000,
    // milliseconds
    custom_links: {},
    // { state: [ url, regexp ] }, for example { php : [ 'doc/$&.html', /\b(getData|setData)\b/g ] }
    api: {},
    // { state: { function: description } }, for example { php: { array: 'Create an array' } }
    ...
    php_function: 'eval|create_function|assert|classkit_method_add|classkit_method_redefine|runkit_function_add|runkit_function_redefine|runkit_lint|runkit_method_add|runkit_method_redefine' + '|array_filter|array_map|array_reduce|array_walk|array_walk_recursive|call_user_func|call_user_func_array|ob_start|sqlite_create_function|is_callable'// callback parameter with possible call of builtin function
    ,
    tr: undefined,
    regexps: undefined,
	...
	*/
    highlight_tag: function(tag, tab_width) {
        ...
        var highlight = function() {
            var start = new Date();
            for (; i < pre.length; i++) {
                var match = /(^|\s)(?:jush|language(?=-\S))($|\s|-(\S+))/.exec(pre[i].className);
                ...
                if (match) {
                    ...
                    // span - enable style for class="language-"
                ...
                for (var i = arguments.length - 4; i > 1; i--) {
                    if (arguments[i]) {
                        ...
                        case 'php_new':
                            link = link.replace(/\$1/g, arguments[i].toLowerCase());
                            break;
                            // toLowerCase() - case sensitive after #
                        case 'phpini':
                            link = link.replace(/\$1/g, arguments[i].toLowerCase().replace(/_/g, '-'));
                            break;
                        case 'php_doc':
                        ...
                        case 'http':
                            if (/-header$/.test(link))
                                link = link.replace(/\$1/g, arguments[i].toLowerCase()).substr(0, 67);
                            break;
                        case 'sql':
    ...
    highlight_states: function(states, text, in_php, escape) {
        if (!this.regexps) {
            var php = /<\?(?!xml)(?:php)?|<script\s+language\s*=\s*(?:"php"|'php'|php)\s*>/i;
            // asp_tags=0, short_open_tag=1
            var num = /(?:\b[0-9]+\.?[0-9]*|\.[0-9]+)(?:[eE][+-]?[0-9]+)?/;
            this.tr = {
                // transitions - key: go inside this state, _2: go outside 2 levels (number alone is put to the beginning in Chrome)
                htm: {
                    php: php,
                    tag_css: /(<)(style)\b/i,
                    tag_js: /(<)(script)\b/i,
                ...
                //! php_quo allows [0-7]{1,3} and x[0-9A-Fa-f]{1,2}
                one: {
                    _1: /(?=\n)|$/
                },
                clr: {
                ...
                //! classes are used also for type hinting and catch //! , because of 'implements' but fails for array(new A, new B)
                php_met: {
                    php_one: /\/\/|#/,
                    php_com: /\/\*/,
                    _1: /()([\w\u007F-\uFFFF]+)()/
                ...
                // php_eot2._2 to be set in php_eot handler
                php_quo: {
                    php_quo_var: /\$\{|\{\$/,
                    php_var: /()(\$[\w\u007F-\uFFFF]+)()/,
                    esc: /\\/,
                ...
                sql_eot2: {},
                // sql_eot2._2 to be set in sql_eot handler
                com_nest: {
                    com_nest: /\/\*/,
                    _1: /\*\//
                },
        ...
        loop: while (start < text.length && (match = this.regexps[state].exec(text))) {
            ...
            for (var key in this.tr[state]) {
                ...
                if (m && !m.index && m[0].length == match[0].length) {
                    // check index and length to allow '/' before '</script>'
                    ...
                    if (/^(att_quo|att_apo|att_val)$/.test(state) && (/^(att_js|att_css|att_http)$/.test(prev_state) || /^\s*javascript:/i.test(s))) {
                        // javascript: - easy but without own state //! should be checked only in %URI;
                        ...
                        s = this.htmlspecialchars(s);
                        s_states = [(escape ? escape(s) : s), (!out || !/^(att_js|att_css|att_http|css_js|js_write_code|js_http_code|php_php|php_sql|php_sqlite|php_pgsql|php_mssql|php_oracle|php_echo|php_phpini|php_http|php_mail)$/.test(state) ? child_states : [])];
                        // reset child states when leaving construct
                    }
                    s = s_states[0];
                    ...
                    if (!out) {
                        ...
                        if (state == 'php_eot') {
                            ...
                            this.regexps.php_eot2 = this.build_regexp((match[2] == "'" ? {
                                _2: this.tr.php_eot2._2
                            } : this.tr.php_eot2));
	...
	*/
    ...
    htmlspecialchars_quo: function(string) {
        ...
        // jush - this.htmlspecialchars_quo is passed as reference
    },
	...
	*/
    html_entity_decode: function(string) {
        return string.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&nbsp;/g, '\u00A0').replace(/&#(?:([0-9]+)|x([0-9a-f]+));/gi, function(str, p1, p2) {
            //! named entities
...
jush.urls = {
    ...
    js: ['https://developer.mozilla.org/en/$key', 'JavaScript/Reference/Global_Objects/$1', 'JavaScript/Reference/Global_Properties/$1', 'JavaScript/Reference/Global_Functions/$1', 'JavaScript/Reference/Statements/$1', 'JavaScript/Reference/Statements/do...while', 'JavaScript/Reference/Statements/if...else', 'JavaScript/Reference/Statements/try...catch', 'JavaScript/Reference/Operators/Special_Operators/$1_Operator', 'DOM/document.$1', 'DOM/element.$1', 'DOM/event.$1', 'DOM/form.$1', 'DOM/table.$1', 'DOM/window.$1', 'http://www.w3.org/TR/XMLHttpRequest/', 'JavaScript/Reference/Global_Objects/Array/$1', 'JavaScript/Reference/Global_Objects/Date/$1', 'JavaScript/Reference/Global_Objects/Function/$1', 'JavaScript/Reference/Global_Objects/Number/$1', 'JavaScript/Reference/Global_Objects/RegExp/$1', 'JavaScript/Reference/Global_Objects/String/$1']
};
...
jush.links = {
    ...
    tag_css: {
        'present/styles': /^(style)$/i
    },
    tag_js: {
        'interact/scripts': /^(script)$/i
    },
    att_css: {
        'present/styles': /^(style)$/i
    },
    ...
    js_http: {
        'method': /^(setRequestHeader|getResponseHeader)$/
    },
    php_new: {
    ...
    php_fun: {
        'http://www.php.net/functions.user-defined': /^function$/i
    },
    php_var: {
        'globals': /^\$GLOBALS$/,
        'server': /^\$_SERVER$/,
    ...
    php_php: {
        'function': new RegExp('^' + jush.php_function + '$','i')
    },
    php_sql: {
        'function': new RegExp('^' + jush.sql_function + '$','i')
    },
    php_sqlite: {
        'function': new RegExp('^' + jush.sqlite_function + '$','i')
    },
    php_pgsql: {
        'function': new RegExp('^' + jush.pgsql_function + '$','i')
    },
    php_mssql: {
    ...
    php_oracle: {
        'function': new RegExp('^' + jush.oracle_function + '$','i')
    },
    php_phpini: {
        'function': /^(ini_get|ini_set)$/i
    },
    php_http: {
        'function': /^header$/i
    },
    ...
    php_echo: {
        'function': /^(echo|print)$/i
    },
    php_halt: {
        'function': /^__halt_compiler$/i
    },
    sql_sqlset: {
        'set-option.html': /.+/
    },
    ...
    cnf_http: {
        'mod_cache': /CacheIgnoreHeaders/i,
        'mod_headers': /.+/
    },
    cnf_php: {
        'configuration.file': /.+/
...
jush.links2 = {
    ...
    php_fun: /(\b)(__autoload|(__construct)|(__destruct)|(__call|__callStatic)|(__get|__set|__isset|__unset)|(__sleep|__wakeup)|(__toString)|(__invoke)|(__set_state)|(__clone))(\b)/i,
    ...
    // collisions: char, set, allow parenthesis - IN, ANY, ALL, SOME, NOT, AND, OR, XOR
